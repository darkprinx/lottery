...
class LotteryEventManager:
    ...
    def is_active_lottery_event(self, lottery_event_id):
        return LotteryEvent.objects.filter(
            pk=lottery_event_id, status=LotteryEventStatus.ACTIVE
        ).exists()

    def is_user_registered(self, user_id, lottery_event_id):
        return LotteryEvent.objects.filter(
            pk=lottery_event_id, participants=user_id
        ).exists()
    ...
    def get_lottery_winner_by_date(self, search_date):
        return (
            LotteryEvent.objects.filter(created_at=search_date)
